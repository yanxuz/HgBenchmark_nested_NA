...
;  if tau0 ge 183336.0 then file=file2
 ...
 if not success then success=ctm_get_datablock(anthr,'HG-SRCE',tracer=1,file=file+'.pt1',tau0=tau0,$
	lat=lat0,lon=lon0,y=latitude,x=longitude,$
	thisdatainfo=datainfo,gridinfo=gridinfo,non=non)
 if not success then success=ctm_get_datablock(anthr,'HG-SRCE',tracer=1,file=file+'.pt2',tau0=tau0,$
	lat=lat0,lon=lon0,y=latitude,x=longitude,$
	thisdatainfo=datainfo,gridinfo=gridinfo,non=non)
...
;  yanxu added diagnositc cven to enhance the convective wet deposition
;  09/27/10
; wetcv_rgm=wetcv_rgm*cven
; wetcv_hgp=wetcv_hgp*cven
...
;      wetdep_cv_all=replicate({rgm:tmp,hgp:tmp},12)
 ...
 conc_all(k).newrgm=newrgm(*,*,0)
 conc_all(k).newrpm=newrpm(*,*,0)
