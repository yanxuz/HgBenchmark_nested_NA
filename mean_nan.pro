...
;        Modified from mean.pro, but now returns nan for values with 
;        on missing data (instead of 0).
;
; CATEGORY:
;        Math & Units
;
; CALLING SEQUENCE:
;        RESULT = MEAN_NAN( X, DIM, _EXTRA=e )
;
; INPUTS:
...
function Mean_Nan, X, Dim, _EXTRA=e
   ...
   result=Total( X, Dim, _EXTRA=e ) / ( Total( Finite( X ), Dim ) > 1 )

   ; replace missing points with NaN
   points= Total( Finite( X ), Dim ) 
   ind = where(points eq 0)
   if ind(0) gt -1 then result(ind)=!values.f_nan
   Return,result
